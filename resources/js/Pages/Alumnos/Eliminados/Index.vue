    ...
    <app-layout>
        <template #header>
            <span class="font-semibold text-md text-gray-800 leading-tight">
                <inertia-link class="hover:underline" :href="route('roles.index', institucion_id)">Roles</inertia-link> / Alumnos eliminados
            </span>
        </template>
...
<script>
    ...
    export default {
        ...
        methods: {
            destroy(id) {
                if (confirm('¿Estás seguro de que deseas eliminar DEFINITIVAMENTE a este alumno?')) {
                    this.$inertia.delete(this.route('alumnoEliminado.destroy', [this.institucion_id, id]))
                }
            },

            buscar() {
                this.$inertia.replace(this.route('alumnoEliminado.index', {institucion_id: this.institucion_id, nombre: this.nombre}))
            }
        }
    }
</script>
